...
class xxHomeController extends Controller
{
    use Wallet;
    private $withdrowAmt = 50;
    private $mBonus = 10;
...
/* ################# Aprove ID     Premium        #########################*/
    ...
    protected function bonusDist($id){
        ...
        if($member->referralId != 0){
            ...
            if($refMember->referralId != 0){      
                ...
                $this->levelBonus($parent,$member->hand);
            }
        }
    }


    protected function levelBonus($parent_id,$hand){          
        $countLeftChild = User::myChild($parent->id, 1);
        $countRightChild = User::myChild($parent->id, 2);

        //echo $parent->id.'-'.$countLeftChild.' '.$countRightChild.'<br>';

        if($hand == 1){
            if($countRightChild >= $countLeftChild){
                $this->joinBonus($parent->id,$bonus);
                //echo 'left';
            }                
        }else{
        ...
        if($parent->referralId){
            $pparent = User::find($parent->referralId);
            if($pparent->admin != 1){
                $this->parent($pparent,$parent->hand);
            }
        }
    }
    ...
    public function upgrateStandrad(){
        ...
        if($balance >= 360) {
            ...
            Session::flash('warning','Sorry, Your Balance Less then 360 Tk');
        }
